...
function url($url) {
  return "http://localhost/visie/pagseguro{$url}";
}
...
class RetornoClass extends UnitTestCase {
  ...
  function testTipoEnviadoAoPrepara() {
    foreach (array('', new stdClass, 123, 12.15) as $item)
      $this->assertIdentical('', RetornoPagSeguro::_preparaDados($item), 'Nao eh aceito este tipo de dado: ' . gettype($item));
  }
  ...
  function testEnviarDadosFsocket() {
    ...
    $tipo = array ('fsocket', '/visie/pagseguro/verificar.php', $socket);
